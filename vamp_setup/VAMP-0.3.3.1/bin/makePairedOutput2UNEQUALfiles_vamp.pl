#!/usr/bin/perl
#RLW 2010
# Updated 2012-01-13 Lance Parsons - Modified to handle Illumina new (CASAVA 1.8+) id format and additonal read numbers
...
while(<IN2>){
   chomp;
   if(/^\>(\S+)(?:[ab1234]| \d:[YN]:\d:[ACGT]*)$/){ # Handle Illumina's new (1.8+) id format and additional read numbers (up to 4)
     $template2=$1;
   }else{
     chomp($_);
     $t2->{$template2}=$_;

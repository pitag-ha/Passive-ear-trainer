...
class RegionAlignment( object ):
    ...
    def add_species( self, species ):
        self.seq_offset[species] = self.nspecies
        self.nspecies += 1
        self._store.seek( self.seq_offset[species] * self.size, 0 )
        self._store.write( "-" * self.size )
    ...
    def get_sequence( self, species ):
        self._store.seek( self.seq_offset[species] * self.size, 0 )
        return self._store.read( self.size )
    ...
    def set_range( self, index, species, bases ):
        ...
        if( index + len(bases)) >self.size : raise Exception ("Your sequence (index %i, %i bases) extends out of range (0 - %i)" % ( index, len(bases), self.size - 1 ) )
        if len( bases ) == 0: raise Exception( "A set of genomic positions can only have a positive length." )
        if species not in self.seq_offset.keys(): self.add_species( species )
        self._store.seek( self.seq_offset[species] * self.size + index, 0 )
        self._store.write( bases )
        
    #Flush temp file of specified species, or all species
    #is this ever used? I don't see it anywhere, so I am changing it to flush everything when called (JAS)
...
class SplicedAlignment( object ):
    ...
    #returns the sequence for a species
    def get_sequence( self, species ):
        #lets assume we can fit a single sequence into memory... 
        #I think returning read() does that anyway, otherwise we would return the filehandle
        # sequence = tempfile.TemporaryFile()
        sequence = ""
        for exon in self.exons:
            if species in exon.get_species_names():
                sequence += exon.get_sequence( species )
            else:
                sequence += ( "N" * exon.size )
        return sequence
